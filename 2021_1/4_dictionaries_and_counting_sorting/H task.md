...
| ------------------- | -------------------------------- | ---------- |
| Ограничение времени | 2 секунды                        | 10 секунд  |
...
Алгоритм скользящего окна для поиска анаграмм:

1. **Подсчет частот** символов в шаблоне $W$
2. **Скользящее окно** длиной $g$ по последовательности $S$
3. **Сравнение частот** - если частоты символов в окне совпадают с частотами шаблона, счетчик +1
4. **Обновление окна** - при движении удаляем левый символ, добавляем правый
5. **Сложность**: $O(|S|)$ времени, $O(\text{количество уникальных символов})$ памяти

**Ключевая идея**: две строки являются анаграммами, если частоты их символов совпадают.

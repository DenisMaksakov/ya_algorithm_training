...
**Суть алгоритма:**
- Используем два указателя: `i` для маек, `j` для штанов
- Оба списка предварительно отсортированы (по условию задачи)
- На каждом шаге:
  1. Сравниваем текущую пару `shirts[i]` и `pants[j]`
  2. Обновляем лучший результат, если нашли меньшую разницу
  3. Двигаем указатель того списка, где текущий элемент **меньше**

**Почему это работает:**
Если `shirts[i] < pants[j]`, то увеличение `i` может уменьшить разницу.
Если `pants[j] < shirts[i]`, то увеличение `j` может уменьшить разницу.

## Сложность

**Временная сложность:** **O(N + M)**
- Каждый указатель проходит свой список только один раз
- Всего делается не более N + M шагов

**Пространственная сложность:** **O(N + M)**
- Для хранения входных данных
- Константная дополнительная память для указателей